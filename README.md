...
### 1. LigaPure — `ComicShannsLigaPure Nerd Font` / `SeriousShannsLigaPure Nerd Font`

The original fonts with only ligatures added and nothing else changed.
...
### 2. Liga — `ComicShannsLiga Nerd Font` / `SeriousShannsLiga Nerd Font`

Same as Pure, but the following characters are replaced with their RecMonoSmCasual equivalents to make it have a consistent look with ligatures:
...
### 3. LigaMod — `ComicShannsLigaMod Nerd Font` / `SeriousShannsLigaMod Nerd Font`

Same as Liga, with additional opinionated replacements from RecMonoSmCasual that I personally like more
